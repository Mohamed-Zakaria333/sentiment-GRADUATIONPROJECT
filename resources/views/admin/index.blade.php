@extends('admin.layouts.master')
@section('sidebar')
    <aside>
      <div id="sidebar" class="nav-collapse ">
        <!-- sidebar menu start-->
      <ul class="sidebar-menu" id="nav-accordion">
          <p class="centered"><a href="profile.html"><img src="{{url($avatars_path)}}/{{Session::get('active_admin')[1]}}" class="img-circle" width="80"></a></p>
          <h5 class="centered"><span style="margin-right: 10px">{{Session::get('active_admin')[2]}}</span>{{Session::get('active_admin')[3]}}</h5>
          <li class="mt">
            <!-- mt -->
            <a class="active" href="{{ route('Editing')}}">
              <!-- fa fa-dashboard -->
              <i class="fa fa-dashboard"></i>
              <span>Edit/Delete</span>
              </a>
          </li>
          <li class="mt">
            <a href="{{ route('Adding')}}">
              <i class="fa fa-desktop"></i>
              <span>ADD product</span>
              </a>
 
          </li>
          <li class="mt">
            <a href="{{ route('Analyting',[Config::get('first_analyting_product')])}}">
              <i class="fa fa-cogs"></i>
              <span>Comment analytic</span>
              </a>
 
          </li>
          <li class="mt">
            <a href="{{ route('Requesting')}}">
              <i class="fa fa-book"></i>
              <span>Product Requests</span>
              </a>
 
          </li>

        </ul>
        <!-- sidebar menu end-->
      </div>
    </aside>



@endsection
@section('content')
   <!--  //========================================================== -->
    <section id="main-content">
      <section class="wrapper">
           <!-- //===================== -->
   @if ($message = Session::get('suc'))
    <div class="alert alert-primary" role="alert">
    product deleted successfuly
     </div>
   @endif
 <!--   //===================================================== -->
        
           


                @foreach($products as $product)
                  <div class="pro1">
                  <img class="img" src="{{url($uploaded_img_path)}}/{{$product->pro_imgPath}}" style="width:300px;height:200px">
              
                <div class="modify" >
                <a href="{{ route('modifing',[$product->id])}}"><button class="product_btn"style="background-color: #e7c855"> Modify</button></a>
                </div>
                <div class="delete" >
                <a id="pro{{$product->id}}" ><button  onclick="desicion('pro{{$product->id}}','{{$product->id}}')"class="product_btn" style="background-color: #e5041e">delete</button></a>
                </div>
                </div>
                 @endforeach
    </section>
  </section>
@endsection
@section('custom_scripts')
<script type="text/javascript">
        function desicion(ementId,pro_id) { 
       

       console.log(pro_id);

            var link=document.getElementById(ementId); 
            var result = confirm("do you want to delete this product"); 
            if (result == true) { 
               
                link.href = "{{url('/Dashboard/products/delete_product/id=')}}"+pro_id;
            } else { 
               
                link.href = "{{ route('Editing')}}";
            } 
            
        } 
</script>
@endsection